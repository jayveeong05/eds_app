...
if (!empty($data->idToken)) {
    ...
    if ($verification['valid']) {
        ...
        if (!$uid || !$email) {
            ...
            echo json_encode(array("message" => "Token valid but missing UID/Email."));
            exit;
        }
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if($stmt->execute()){
                ...
                echo json_encode(array("message" => "Unable to create user in DB."));
            }
        }
        
    } else {
        http_response_code(401);
        echo json_encode(array("message" => "Invalid ID Token: " . $verification['error']));
    }
} else {
    http_response_code(400);
